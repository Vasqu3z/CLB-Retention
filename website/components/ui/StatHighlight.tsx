    ...
    return (
      ...
      >
        <div
          ...
          style={{
            background: `radial-gradient(circle at 20% 20%, ${glowColor}55, transparent 45%)`
          }}
        />
        <div className="relative space-y-2">
          <div className="text-xs uppercase tracking-[0.25em] text-white/60 font-ui">{label}</div>
          <div className="font-display text-3xl text-white drop-shadow-[0_0_12px_rgba(255,255,255,0.25)]">{value ?? "—"}</div>
          {subtext && <div className="font-mono text-xs text-white/60">{subtext}</div>}
        </div>
      </motion.div>
    );
  }
    ...
    <section className="py-24 md:py-32 container mx-auto px-4 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div className="relative">
          ...
          {/* Background elements */}
          <div className="absolute -inset-8 bg-gradient-radial from-comets-cyan/20 via-transparent to-transparent blur-3xl" />
          <div className="absolute -inset-12 bg-gradient-conic from-white/10 via-transparent to-transparent animate-spin-slow" />
        </div>
